...
export default function DashboardPage() {
    ...
    return (
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8" onClick={() => setContextMenu(null)}>
            {/* Main Content Area */}
            ...
            >
                {[
                    { label: 'Total Folders', value: folders.length, icon: FolderOpen, color: 'text-primary-400' },
                    { label: 'Documents', value: folders.reduce((acc, f) => acc + (f.documents?.length || 0), 0), icon: FileText, color: 'text-primary-300' },
                    { label: 'Last Upload', value: 'Today', icon: Clock, color: 'text-surface-300' },
                    { label: 'Account', value: session?.user?.accountType === 'family' ? 'Family' : 'Individual', icon: null, color: 'text-surface-300' },
                ].map((stat) => (
            ...
            >
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-5 py-3 bg-primary-600 hover:bg-primary-500 text-white rounded-xl font-medium transition-all"
                >
