...
export default function FolderDetailPage() {
    ...
    const handleUpload = async (uploadData) => {
        try {
            // uploadData from ScanUploadFAB already contains folderId, title, and report
            // Route through Next.js proxy which handles auth via session
    ...
    return (
        <div className="max-w-6xl mx-auto">
            ...
            {/* Scan & Upload FAB — locked to current folder */}
            <ScanUploadFAB
                folders={allFolders}
                onUpload={handleUpload}
                lockedFolderId={params.id}
                lockedFolderName={folder?.name}
            />
        </div>
    );
}
